...
const FALLBACK_DELIVERY_TAG = "v1.26.6";
  ...
  "v1.26.6": [
    {
      title: "新增",
      items: [
        "自动化服务新增持久化恢复状态与健康轮询；归档准入未恢复时，创建和重搜入口会显示恢复提示并暂时禁用，恢复后自动重新开放。",
      ],
    },
    {
      title: "改进",
      items: [
        ...
        "新任务和重搜在持久化归档短暂不可用时会先进行最多 8 秒的有界恢复等待；仍未就绪时返回明确的中文提示和重试时间，并保留请求幂等键与批量选择。",
      ],
    },
    {
      title: "修复",
      items: [
        ...
        "修复大批次在不同分页和异常筛选下进度统计口径不一致的问题，并补齐持久化准入状态在服务监控、Agent Search 健康检查和前端操作入口之间的一致性。",
      ],
    },
    {
      title: "运维 / 配置",
      items: [
        ...
        "GitHub `releases/latest` 仍停留在 v1.24.0，安装服务继续以真实 tag 列表判定 v1.26.6 为最新交付版本。",
      ],
    },
    {
      title: "迁移与兼容性提示",
      items: [
        "本版本不包含数据库迁移，也不会改写历史任务或结果；滚动发布时建议先更新 API 再更新界面，旧界面会安全忽略新增响应字段。",
        "本次网站更新不会连接或修改客户数据库；如需回滚，可直接回到 v1.26.5。",
      ],
    },
  ],
  "v1.26.5": [
    ...
    },
    {
      title: "改进",
      items: [
        "失败或停止物料改为在服务端完成全轮次聚合后再分页；单轮次“仅看异常”和表头全选可覆盖全部异常物料，不再受当前页限制。",
        "完全匹配只采信商品标题与对象门禁，商家公司名、主营范围等弱证据仅作为人工确认线索；数量不足时展示真实缺口，不再用重复、无身份或错品类候选凑数。",
      ],
    },
    {
      title: "修复",
      items: [
        "修复已完成对账的记录仍占用异常页、异常重跑跨页漏选，以及“全部轮次”下分类汇总数量不准确的问题。",
        "修复严格准确率高于 80% 的已标记物料被误显示为未标记的问题；标记状态和重搜资格现在分别统计，高准确率物料仍不会进入批量标记重搜。",
      ],
    },
    {
      title: "运维 / 配置",
      items: [
        "同步 v1.26.5 客户 `.env` 配置；安装器会在每次安装时通过受保护的部署环境变量刷新共享环境文件。",
        "GitHub `releases/latest` 仍停留在 v1.24.0，安装服务继续以真实 tag 列表判定 v1.26.5 为最新交付版本。",
      ],
    },
    {
      title: "迁移与兼容性提示",
      items: [
        "本版本新增生产 schema baseline，并调整两项既有迁移的执行顺序；升级前应先核对结构指纹，再按交付顺序应用迁移。",
        "本次网站更新不会连接或修改客户数据库；数据库基线与迁移仍需在授权环境中由发布负责人显式执行。",
      ],
    },
  ],
  "v1.26.4": [
    ...
    },
    {
      title: "改进",
      items: [
        "批量标记重搜按严格准确率筛选，超过 80% 的商品不再计入，恰好 80% 仍可重搜；确认弹窗会展示完整商品名、准确率和已标记数量。",
    ...
    },
    {
      title: "修复",
      items: [
        "修复保存失败或响应不一致时误报成功、刷新后丢失审核草稿的问题，并在切换工作区、关闭复核或离开页面前保护未保存内容。",
        "修复取消放弃未保存审核后确认框重复弹出的问题；取消会保留当前页面、物料和复核状态，只有明确放弃后才继续原操作。",
        "修复“全部轮次”物料数取单轮最大值的问题，搜索进度和商品复核入口现在均展示各轮次累计总数。",
      ],
    },
    {
      title: "运维 / 配置",
      items: [
        "同步 v1.26.4 客户 `.env` 配置；安装器会在每次安装时通过受保护的部署环境变量刷新共享环境文件。",
        "GitHub `releases/latest` 仍停留在 v1.24.0，安装服务继续以真实 tag 列表判定 v1.26.4 为最新交付版本。",
      ],
    },
    {
      title: "迁移与兼容性提示",
      items: [
        "本版本不包含数据库迁移，升级后可直接使用新的审核保存保护和批量重搜校验。",
        "本次网站更新不会连接或修改客户数据库。",
      ],
    },
  ],
  ...
  "v1.25.0": [
    {
      ...
      items: [
        ...
        "新增已标记物料选择、批量审核与批次重跑能力，可从进度列表选择符合条件的物料并集中发起后续处理。",
        "新增商品卡片图片缓存与跨机器回建能力，本地证据缺失时可基于已持久化审核结果恢复预览，并保留纯文本审核入口。",
        "新增自动化服务管理员重启与受管健康控制能力，服务异常时可通过统一状态入口请求安全重启。",
      ],
    },
    {
      title: "改进",
      items: [
        "优化 Agent Search 工作台布局、批次选择器、轮次筛选和刷新控件，默认聚合全部轮次，并将自动倒计时与手动刷新合并为单一操作。",
        "优化物料与批次列表的最新文件识别、筛选和选择状态，减少同名批次、旧文件和重跑层级对日常操作的干扰。",
        "优化商品卡片图片恢复、查询缓存和结果归一化，降低重复生成缩略图和证据缺失造成的审核阻塞。",
        "优化自动刷新、Realtime 查询与自动化服务状态提示，在后台异常或刷新降级时提供更明确的用户反馈。",
      ],
    },
    {
      title: "修复",
      items: [
        "修复待处理写入持续积压时仍继续放行新任务的问题；健康断路器会在超过阈值时阻止新工作并保留恢复诊断。",
        "修复自动化服务重启、就绪检查和全局并发许可状态不同步时可能导致的错误状态或重复操作。",
      ],
    },
    {
      title: "运维 / 配置",
      items: [
        "更新 Agent Search 审核、并发容量、健康断路器、卡片缓存和自动化服务控制相关 `.env` 配置；生产部署需同步本次提供的 `.env` 文件。",
        "自动化服务重启必须通过受管控制入口执行；服务未就绪或 pending 健康门禁触发时继续采用 fail-closed 行为。",
      ],
    },
    {
      title: "迁移与兼容性提示",
      items: [
        "本版本新增自动化并发容量扩展和 Agent Search 批量审核数据库迁移；升级前需通过既有安全发布入口按顺序执行并保留审计回执。",
        "本次网站更新不会连接或修改客户托管数据库，远端迁移仍需由发布负责人在授权环境中显式执行。",
      ],
    },
  ],
  "v1.24.0": [
