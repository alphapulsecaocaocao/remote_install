  ...
  it("uses release metadata when the latest release matches the newest tag", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          tag_name: "v1.23.0.preview",
          html_url:
            "https://github.com/yueyue27418/1688-autoprocurement/releases/tag/v1.23.0.preview",
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
    ...
    expect(latest).toMatchObject({
      source: "release",
      tagName: "v1.23.0.preview",
      archiveUrl:
        "https://1688autoprocurement.xleeelx.online/api/downloads/tags/v1.23.0.preview",
    });
  });
...
describe("getDeliveryVersions", () => {
  it("lists valid delivery tags from v1.15.1 onward in newest-first order", async () => {
    ...
    expect(versions[11]).toMatchObject({
      archiveUrl:
    ...
    expect(JSON.stringify(versions[11]?.changelog.sections)).not.toContain(
      "automation/",
    );
