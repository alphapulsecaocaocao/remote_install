  ...
  it("uses release metadata when the latest release matches the newest tag", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          tag_name: "v1.18.0.alpha",
          html_url:
            "https://github.com/yueyue27418/1688-autoprocurement/releases/tag/v1.18.0.alpha",
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
    ...
    expect(latest).toMatchObject({
      source: "release",
      tagName: "v1.18.0.alpha",
      archiveUrl:
        "https://1688autoprocurement.xleeelx.online/api/downloads/tags/v1.18.0.alpha",
    });
  });
...
describe("getDeliveryVersions", () => {
  it("lists valid delivery tags from v1.15.1 onward in newest-first order", async () => {
    ...
    expect(JSON.stringify(versions[4]?.changelog.sections)).not.toContain(
      "automation/",
    );
