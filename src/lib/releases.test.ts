...
describe("getLatestDeliveryVersion", () => {
  it("uses release metadata when the latest release matches the newest tag", async () => {
    ...
    expect(latest).toMatchObject({
      source: "release",
      tagName: "v1.17.6.fix.alpha",
      archiveUrl:
        "https://1688autoprocurement.xleeelx.online/api/downloads/tags/v1.17.6.fix.alpha",
    });
  });
...
describe("getDeliveryVersions", () => {
  it("lists valid delivery tags from v1.15.1 onward in newest-first order", async () => {
    ...
    expect(versions[3]?.changelog.sections).toEqual([
      {
        title: "改进",
        items: [
    ...
    expect(JSON.stringify(versions[3]?.changelog.sections)).not.toContain(
      "automation/",
    );
